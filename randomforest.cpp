...
Node* randomForest(const vector<vector<double>>& dataset){    
    Node* rootNode = new Node(0);
    rootNode->dataSet = dataset;
    int dataSize = dataset.size();
    int proportion = dataSize/30;
    ...
    while(!nodeQueue.empty()){
        ...
        dataSize = nodeQueue.front()->dataSet.size();
        int n = sqrt(dataSize);
        vector<vector<double>> ranData = randomSample(nodeQueue.front()->dataSet,n);
        vector<vector<double>> data = nodeQueue.front()->dataSet;
        Split* split = new Split;
        split = bestsplit(ranData);
        ...
        double varDiff = squareDifference(callTargets(ranData)) - var;
        ...
        if(data.size()<=proportion){ //second terminating condition
            ...
            nodeQueue.front()->predictedValue = calculateMean(callTargets(ranData));
            ...
            if(leaf->parent!=nullptr){
                ...
                if(parent->left!=nullptr && parent->right!=nullptr){
                    ...
                    if(parent->left->left==nullptr && parent->left->right==nullptr && parent->right->left==nullptr && parent->right->right==nullptr){
                        ...
                        //I did totalError-0.5 to speed up the program
...
double kFoldCrossValidation(Node* root, const vector<vector<double>>& trainingData, int k){
    ...
    for(double alpha=0;alpha<2;alpha+=0.2){
        ...
        for(int i=0;i<k;i++){
            ...
            Node* trainedTree = randomForest(trainingSet);
...
int main(int argc, char* argv[]) {
    ...
    Node* decision = randomForest(trainingData);
    ...
    plotVectorOfVectors(leaf,"aaplforest.pdf");
    delete decision;
